...
namespace ShouPiao
{
    public class DbSqlHelper
    {
        ...
        protected static string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["yp_conn"].ConnectionString;
        ...
        public static int RunProcedure(object target, out Dictionary<string, object> outs)
        {
            //思路：使用反射的方式来获取所有属性，属性名就是参数名，属性值为参数值，属性类型为参数类型
            List<SqlParameter> paras = new List<SqlParameter>();
            SqlParameter para = null;
            Type t = target.GetType();//获得类型
            PropertyInfo proc = t.GetProperty("Name");//获得name属性
            string procName = proc.GetValue(target, null).ToString();//得到属性值

            var props = t.GetProperties(BindingFlags.Instance | BindingFlags.Public);//得到所有公有实例属性
            ...
            int rowsaffected;
            int ret = RunProcedure(procName, paras.ToArray(), out rowsaffected);
