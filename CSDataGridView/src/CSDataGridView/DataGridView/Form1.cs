...
namespace DataGridView
{
    public partial class Form1 : Form
    {
        ...
        private BindingList<Value4> lst = new BindingList<Value4>();
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            var indices = selRows.Select(x => x.Index).OrderBy(x => x).ToList();

            this.dataGridView1.DataSource = null;

            for (int i = indices.Count - 1; i >= 0; i--)
            {
                this.lst.RemoveAt(indices[i]);
            }
            this.dataGridView1.DataSource = lst;
            this.ShowRowNumber();
        }
        ...
        private void ReadAll()
        {
            ...
            {
                this.lst =new BindingList<Value4>( File.ReadAllText(fileName, Encoding.UTF8)
